  ...
  return (
    <div className="flex items-center justify-center gap-1.5 mt-10 flex-wrap">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#121622] border border-[#202838] text-white disabled:opacity-30 hover:border-[#3B82F6] hover:bg-[#1a2035] transition-all disabled:cursor-not-allowed text-sm font-medium"
      >
      ...
      {getPageNumbers().map((page, idx) =>
        page === "..." ? (
          <span key={`e-${idx}`} className="px-2 text-[#B0B8D0]">…</span>
        ) : (
          <button
            key={page}
            ...
            className={`w-9 h-9 rounded-lg text-sm font-bold transition-all ${
              currentPage === page
                ...
                : "bg-[#121622] border border-[#202838] text-white hover:border-[#3B82F6] hover:bg-[#1a2035]"
            }`}
          >
            {page}
          </button>
...
function GameCard({ game, priority = false }: { game: Game; priority?: boolean }) {
  ...
  return (
    <div
      ...
      className="group relative bg-[#0e1221] rounded-xl overflow-hidden border border-[#1e2640] hover:border-[#3B82F6]/60 transition-all duration-300 hover:shadow-[0_0_28px_rgba(59,130,246,0.18)] cursor-pointer flex flex-col"
    >
      <div className="relative aspect-[3/4] w-full overflow-hidden">
        ...
        {game.discount && game.discount > 0 ? (
          <div className="absolute top-2 right-2 bg-[#3B82F6] text-white text-xs font-black px-2 py-1 rounded-md shadow-lg">
            -{game.discount}%
          </div>
        ) : null}
        {game.price === 0 ? (
        ...
        <div className="absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[#0e1221] to-transparent" />
      </div>
      <div className="p-2.5 flex flex-col gap-1.5 flex-1">
        <h3 className="text-white font-semibold text-sm leading-tight line-clamp-2 group-hover:text-[#3B82F6] transition-colors">
          {game.title}
        </h3>
        <div className="flex items-baseline gap-1.5 mt-auto">
          ...
          {game.originalPrice && game.originalPrice > game.price ? (
            <span className="text-[#6B7BA4] text-xs line-through">₹{game.originalPrice}</span>
          ) : null}
        </div>
        <button
          ...
          className={`w-full py-2.5 rounded-lg text-xs font-bold transition-all mt-1 flex items-center justify-center gap-1.5 h-10 ${
            inCart
              ...
              : "bg-[#3B82F6] text-white hover:bg-[#2563EB] hover:shadow-[0_0_12px_rgba(59,130,246,0.5)]"
          }`}
        >
          {inCart ? (
...
function SkeletonCard() {
  return (
    <div className="bg-[#0e1221] rounded-xl overflow-hidden border border-[#1e2640] animate-pulse">
      <div className="aspect-[3/4] w-full bg-[#1a2035]" />
      <div className="p-2.5 space-y-2">
        <div className="h-4 bg-[#1a2035] rounded w-3/4" />
        <div className="h-3 bg-[#1a2035] rounded w-1/2" />
        <div className="h-7 bg-[#1a2035] rounded w-full mt-2" />
      </div>
    </div>
  );
}
...
function FilterSection({
  ...
  return (
    <div className="border-b border-[#1e2640] pb-4">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full py-2 text-left group"
      >
        <span className="text-[#7B8DB0] text-xs font-semibold uppercase tracking-wider group-hover:text-white transition-colors">
          {title}
        </span>
        {open ? (
          <ChevronUp className="w-3.5 h-3.5 text-[#4B5680] group-hover:text-white transition-colors" />
        ) : (
          <ChevronDown className="w-3.5 h-3.5 text-[#4B5680] group-hover:text-white transition-colors" />
        )}
      </button>
    ...
    <main className="min-h-screen bg-[#080A10]">
      ...
      <div className="pt-6 pb-16">
        <div className="mx-auto max-w-[1600px] px-4 md:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between gap-3 mb-4">
            <div>
              <h1 className="text-xl sm:text-3xl lg:text-4xl font-black text-white flex items-center gap-2 sm:gap-3">
                <LayoutGrid className="w-5 h-5 sm:w-7 sm:h-7 text-[#3B82F6]" />
                Browse
              </h1>
          ...
          <div
            ref={gridTopRef}
            className="hidden sm:flex flex-wrap items-center gap-3 mb-3 bg-[#0d1120] border border-[#1e2640] rounded-xl px-4 py-3"
          >
            ...
            <button
              ...
              className={`hidden sm:flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${
                filtersOpen
                  ...
                  : "bg-[#121830] border border-[#1e2640] text-[#9BAACB] hover:text-white hover:border-[#3B82F6]"
              }`}
            >
              ...
              {activeFilterCount > 0 && (
                <span
                  className={`text-xs font-black w-5 h-5 rounded-full flex items-center justify-center ${
                    filtersOpen ? "bg-white text-[#3B82F6]" : "bg-[#3B82F6] text-white"
                  }`}
                >
                  {activeFilterCount}
                </span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[#6B7BA4] text-sm hidden md:block">Sort:</span>
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button className="flex items-center gap-1.5 px-3 py-2 bg-[#121830] border border-[#1e2640] rounded-lg text-white text-sm hover:border-[#3B82F6] hover:text-white transition-all cursor-pointer">
                    <span className="text-[#6B7BA4] text-xs sm:hidden">Sort:</span>
                    <span className="font-semibold">{currentSort.label}</span>
                    <ChevronDown className="w-3.5 h-3.5 text-[#6B7BA4]" />
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="bg-[#121622] border border-[#202838] text-white rounded-lg shadow-2xl p-1 w-48 z-50">
                  <DropdownMenuLabel className="text-xs text-[#6B7BA4] px-2 py-1.5 uppercase font-black tracking-wider">
                    Sort By
                  </DropdownMenuLabel>
                    ...
                    {SORT_OPTIONS.map((opt) => {
                      ...
                      return (
                        <DropdownMenuRadioItem
                          key={key}
                          value={key}
                          className="flex items-center justify-between text-xs px-2 py-1.5 hover:bg-white/5 cursor-pointer rounded transition-colors text-[#9BAACB] data-[state=checked]:text-[#3B82F6] data-[state=checked]:font-bold"
                        >
                          {opt.label}
                        </DropdownMenuRadioItem>
                      );
          ...
          >
            <div className="bg-[#0d1120] border border-[#1e2640] rounded-xl p-5">
              {/* Filter Panel Header */}
              <div className="flex items-center justify-between mb-4">
                ...
                {activeFilterCount > 0 && (
                  <button
                    ...
                    className="text-[#3B82F6] hover:text-[#60a5fa] text-xs font-semibold transition-colors"
                  >
                        ...
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                              isChecked
                                ...
                                : "border-[#3B4A6B] group-hover:border-[#5B72A0]"
                            }`}
                          >
                          ...
                          <span
                            className={`text-xs transition-colors truncate ${
                              isChecked ? "text-white font-medium" : "text-[#9BAACB] group-hover:text-white"
                            }`}
                          >
                            {genre}
                          </span>
                        ...
                        >
                          <div
                            className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                              isSelected ? "border-[#3B82F6]" : "border-[#3B4A6B] group-hover:border-[#5B72A0]"
                            }`}
                          >
                            {isSelected && <div className="w-1.5 h-1.5 rounded-full bg-[#3B82F6]" />}
                          </div>
                          <span
                            className={`text-xs transition-colors ${
                              isSelected ? "text-white font-medium" : "text-[#9BAACB] group-hover:text-white"
                            }`}
                          >
                            {range.label}
                          </span>
                  ...
                  <label
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-lg cursor-pointer transition-all border ${
                      onSaleOnly
                        ...
                        : "border-[#1e2640] hover:border-[#3B4A6B]"
                    }`}
                  >
                    <div
                      className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        onSaleOnly ? "bg-[#3B82F6] border-[#3B82F6]" : "border-[#3B4A6B]"
                      }`}
                    >
                      {onSaleOnly && <Check className="w-2 h-2 text-white" />}
                    </div>
                    <Tag className={`w-3.5 h-3.5 ${onSaleOnly ? "text-[#3B82F6]" : "text-[#6B7BA4]"}`} />
                    <span className={`text-xs font-medium ${onSaleOnly ? "text-white" : "text-[#9BAACB]"}`}>
                      On Sale Only
                    </span>
                  </label>
                </FilterSection>
          ...
          {activeFilterCount > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {selectedGenres.map((genre) => (
                <div
                  key={genre}
                  className="flex items-center gap-1.5 bg-[#3B82F6]/15 border border-[#3B82F6]/40 rounded-full px-3 py-1 text-sm text-white"
                >
                  <span>{genre}</span>
                  <button
                    onClick={() => toggleGenre(genre)}
                    className="text-[#9BAACB] hover:text-white transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
              ...
              {priceRange !== "all" && (
                <div className="flex items-center gap-1.5 bg-[#3B82F6]/15 border border-[#3B82F6]/40 rounded-full px-3 py-1 text-sm text-white">
              ...
              <button
                onClick={clearAllFilters}
                className="text-[#6B7BA4] hover:text-[#3B82F6] text-sm font-medium transition-colors px-1"
              >
                Clear all
              </button>
            </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <div className="w-16 h-16 bg-[#0d1120] rounded-2xl flex items-center justify-center mb-4 border border-[#1e2640]">
                <Search className="w-8 h-8 text-[#3B4A6B]" />
              </div>
              <h3 className="text-white font-bold text-xl mb-2">No games found</h3>
              <p className="text-[#6B7BA4] text-sm mb-4 max-w-xs">
          ...
          <div className="fixed bottom-6 right-6 z-40 sm:hidden">
            <Sheet>
              <SheetTrigger asChild>
                <button className="relative w-14 h-14 bg-[#3B82F6] hover:bg-[#2563EB] text-white rounded-full shadow-[0_8px_32px_rgba(59,130,246,0.4)] flex items-center justify-center border border-white/10 active:scale-95 transition-all cursor-pointer">
                  <SlidersHorizontal className="w-6 h-6" />
                  {activeFilterCount > 0 && (
                    <span className="absolute -top-1 -right-1 text-[10px] font-black w-5.5 h-5.5 rounded-full flex items-center justify-center bg-red-500 border-2 border-[#080A10] text-white">
                      {activeFilterCount}
                    </span>
                  )}
                </button>
              </SheetTrigger>
              <SheetContent side="right" className="w-[300px] sm:w-[360px] bg-[#080A10] border-l border-[#1e2640] p-5 text-white overflow-y-auto flex flex-col h-full z-50">
                <SheetHeader className="pb-3 border-b border-[#1e2640] flex flex-row items-center justify-between">
                  <SheetTitle className="text-base font-black text-white">Filter & Sort</SheetTitle>
                  {activeFilterCount > 0 && (
                    <button
                      onClick={clearAllFilters}
                      className="text-[#3B82F6] hover:text-[#60a5fa] text-xs font-semibold transition-colors mr-6"
                    >
                      Clear all
                    </button>
                  )}
                ...
                <div className="flex-1 space-y-5 py-4 overflow-y-auto pr-1 scrollbar-hide">
                  ...
                  <div>
                    <span className="text-[#7B8DB0] text-xs font-semibold uppercase tracking-wider block mb-2">
                      Sort By
                    </span>
                    <div className="space-y-1 bg-[#121622] border border-[#1e2640] rounded-xl p-3">
                      {SORT_OPTIONS.map((opt) => {
                        const key = opt.label.toLowerCase().replace(/\s/g, "-");
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`text-xs transition-colors ${
                                isSelected ? "text-[#3B82F6] font-bold" : "text-[#9BAACB] group-hover:text-white"
                              }`}
                            >
                              {opt.label}
                            </span>
                            <div
                              className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 transition-all ${
                                isSelected ? "border-[#3B82F6]" : "border-[#3B4A6B]"
                              }`}
                            >
                  ...
                  <div className="border-t border-[#1e2640] pt-4">
                    ...
                    {allGenres.length > 8 && (
                      <button
                        ...
                        className="text-[#3B82F6] hover:text-[#60a5fa] text-xs font-semibold mt-2.5 transition-colors block text-left cursor-pointer"
                      >
                  ...
                  {/* Price Range */}
                  <div className="border-t border-[#1e2640] pt-3">
                    <span className="text-[#7B8DB0] text-xs font-semibold uppercase tracking-wider block mb-2">
                      Price Range
                    </span>
                      ...
                      {PRICE_RANGES.map((range) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span
                              className={`text-xs transition-colors ${
                                isSelected ? "text-white font-medium" : "text-[#9BAACB]"
                              }`}
                            >
                              {range.label}
                            </span>
                  ...
                  {/* Special Offers */}
                  <div className="border-t border-[#1e2640] pt-3 pb-8">
                    <span className="text-[#7B8DB0] text-xs font-semibold uppercase tracking-wider block mb-2">
                      Special Offers
                    </span>
                    <label
                    ...
                    >
                      ...
                      <Tag className={`w-4 h-4 ${onSaleOnly ? "text-[#3B82F6]" : "text-[#6B7BA4]"}`} />
                      <span className={`text-xs font-bold ${onSaleOnly ? "text-white" : "text-[#9BAACB]"}`}>
                        On Sale Only
                      </span>
                    </label>
                  </div>
                ...
                <div className="pt-3 border-t border-[#1e2640] mt-auto">
                  <SheetClose asChild>
                    <button className="w-full py-3 bg-[#3B82F6] hover:bg-[#2563EB] text-white text-sm font-bold rounded-lg transition-colors shadow-[0_4px_16px_rgba(59,130,246,0.3)] cursor-pointer flex items-center justify-center gap-2">
      ...
      fallback={
        <main className="min-h-screen bg-[#080A10]">
          <div className="pt-20 flex items-center justify-center">
            <div className="animate-pulse text-[#6B7BA4]">Loading...</div>
          </div>
        </main>
      }
    >
